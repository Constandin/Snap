...
public class Logic implements Serializable {
    ...
    public static boolean win() {
        ...
        if (xIndex.size() >= 5 && (Collections.max(xIndex) - Collections.min(xIndex)) == Global.LINE_GAP * (xIndex.size() - 1)) {
            isWin = true;
        }
        ...
        if (yIndex.size() >= 5 && (Collections.max(yIndex) - Collections.min(yIndex)) == Global.LINE_GAP * (yIndex.size() - 1)) {
            isWin = true;
        }
        ...
        if (obliqueUpIndex.size() >= 5 && (Collections.max(obliqueUpIndex) - Collections.min(obliqueUpIndex)) == Global.LINE_GAP * (obliqueUpIndex.size() - 1)) {
            isWin = true;
        }
        ...
        if (obliqueDownIndex.size() >= 5 && (Collections.max(obliqueDownIndex) - Collections.min(obliqueDownIndex)) == Global.LINE_GAP * (obliqueDownIndex.size() - 1)) {
            isWin = true;
        }
        return isWin;
    }
    ...
    public boolean dropAndWin(Circle circle) {
...
//        if(isOver) {
//            NetUtils.sendMessage(new WinMessage());
//        }
        return isOver;
    }
